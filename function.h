...
void MainWindow::setGridSize(int size)
{
    ...
    /// qDebug()<<"size:"<<myGridSize;
     if(scene->sceneGridYatay) gridYatayButtonClick();
     if(scene->sceneGridDikey) gridDikeyButtonClick();
     if(scene->sceneGuzelYazi) zeminGuzelYaziButtonClick();
...
if(zamanlamastart!=0)
{
      ...
      scene->addItem(text1);
      text1->setPos(this->width()-2, this->height()-2);
     //}
