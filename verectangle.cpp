   ...
   // painter->drawPolygon(result);
    ...
    //painter->drawRect(rect);
      ...
      /*  QImage resim;
        QPainter par(&resim);
        //painter->begin;
        //par.setWindow(QRect(0, 0, sx-ex, sy-ey));
          par.setBrush(QBrush(Qt::red));
          par.setPen(Qt::green);
          par.drawPath(myPath);
          par.end();
          //addPixmap(QPixmap::fromImage(resim,Qt::AutoColor));
        myImage=QPixmap::fromImage(resim,Qt::AutoColor);
        painter->drawPixmap(0,0,myImage.scaled(100,200,Qt::IgnoreAspectRatio,Qt::SmoothTransformation));
    //painter->drawRect(rect);
