...
/***********************************/
   ...
   /* this->hoverEventFilter = new HoverEventFilter(this);
    ...
    buyukKutu->installEventFilter(this->hoverEventFilter);
/**************************************************************************/
 ...
 //palette->setColor(QPalette::Window, QColor(128,128,128,50));
   ...
   /// pageListwg->setPalette(*palette);
    ...
    screenbtn->setObjectName("screenbtn");

    screenbtn->setStyleSheet("QWidget#screenbtn{"
                          "border: 0.5px solid rgb(62, 140, 220,20);"
                          "border-radius: 5px;"
 ...
 connect(screenbtn, &QPushButton::clicked, [=]() {
    ...
    for(int i=0;i<pageList.size();i++)
    {
        ...
        pageList[i]->setAutoFillBackground(true);
         pageList[i]->setObjectName("screenbtn");

         pageList[i]->setStyleSheet("QWidget#screenbtn{"
                              "border: 0.5px solid rgb(62, 140, 220,20);"
                              "border-radius: 5px;"
...
bool MainWindow::eventFilter(QObject *watched, QEvent *event)
{
    ...
    switch(t){
    case QEvent::Enter:
        emit onHoverIn(watched);
        break;

    case QEvent::Leave:
        emit onHoverOut(watched);
        break;
    default:
        return false;
    }
...
void MainWindow::onHoverIn(QObject *object)
{
    ...
    if(objectName=="buyukKutu")
    {
       //  qDebug()<<"girildi"<<objectName;
        // setCursor(Qt::ArrowCursor);
    }
   /// currentScreenModeSlot();
}
void MainWindow::onHoverOut(QObject *object)
{
    QString objectName = object->objectName();

    if(objectName=="buyukKutu")
    {
        // qDebug()<<"çıkıldı"<<objectName;
